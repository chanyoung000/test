...
sigungu_dict = {
    ...
    "충청남도": ["천안시 동남구", "천안시 서북구", "공주시", "보령시", "아산시",
             ...
             "청양군", "홍성군", "예산군", "태안군"],
    "전라북도": ["전주시 완산구", "전주시 덕진구", "군산시", "익산시", "정읍시",
...
def create_map_with_outer_and_inner_layers(geojson, address_styles, stroke_color='#FFA500', fill_color='white', fill_opacity=0.2):
    ...
    outer_layer = folium.FeatureGroup(name="외곽선만(내부경계 없음)")
    folium.GeoJson(
        merged_geojson,
        name="외곽선만(내부경계 없음)",
...
def run_gui():
    ...
    btn_generate2 = tk.Button(frame_left, text="지도 생성2", width=btn_width, height=btn_height)
    btn_excel_save = tk.Button(frame_left, text="엑셀 저장", width=btn_width, height=btn_height)

    btn_generate2.grid(row=1, column=1, padx=2, pady=2, sticky="we")
    ...
    btn_generate2.config(command=generate_map)
    ...
    frame_right = tk.LabelFrame(frame_right_container, text="스타일 설정", bd=2, relief="solid", height=270)
